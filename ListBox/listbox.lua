...
function listbox:calc(y)
	self.scrypos=y
	local pos=y-self.y
	local c = math.floor(pos*(#self.str)/self.h)

	if c>self.max-1 then
...
function listbox:draw()
	...
	if not listbox.isexpanding and self.expand and x>=self.x+self.w and x<=self.x+self.w+self.expx+1 and y>=self.y+self.h and y<=self.y+self.h+self.expx+1 and self.ispress then
	self.asize=false
	...
	if not listbox.isexpanding and (x>self.x and x<self.x+self.w-self.radius) and (y>self.y+self.h-self.radius*1.1 and y<self.y+self.h+self.radius) and self.ispress and self.fwmaxshow>0 and self.enabled then
	self.scrhpos=x
	local pos=x-self.x
